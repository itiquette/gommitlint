...
// Ensure GitHubActionsFormatter implements Formatter.
var _ Formatter = GitHubActionsFormatter{}
...
func (f GitHubActionsFormatter) Format(_ context.Context, results interface{}) string {
	...
	for i, commitResult := range validationResults.Results {
		...
		for _, ruleResult := range commitResult.RuleResults {
			...
			// Generate message on-the-fly based on verbosity
			message := Result(ruleResult.RuleName, ruleResult.Errors)
			if f.verbose {
				message = VerboseResult(ruleResult.RuleName, ruleResult.Errors)
			}
			...
			if f.showHelp {
				helpMessage := Help(ruleResult.RuleName, ruleResult.Errors)
				if helpMessage != "" {
...
func (f GitHubActionsFormatter) ContentType() string {
	return "text/plain"
}
	...
	for _, commitResult := range results.Results {
		for _, ruleResult := range commitResult.RuleResults {
			if ruleResult.RuleName == ruleName {
				found = true
				// Generate help message on-the-fly
				helpMessage := Help(ruleResult.RuleName, ruleResult.Errors)
