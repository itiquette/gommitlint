...
func (l Loader) LoadFromPath(configPath string) (types.Config, error) {
	...
	if err := koanfConfig.Load(file.Provider(configPath), yaml.Parser()); err != nil {
		return types.Config{}, fmt.Errorf("error loading config file %s: %w", configPath, err)
	}
	...
	if err := koanfConfig.UnmarshalWithConf("gommitlint", &config, koanf.UnmarshalConf{Tag: "yaml"}); err != nil {
		return types.Config{}, fmt.Errorf("error unmarshaling config for gommitlint namespace: %w", err)
	}
