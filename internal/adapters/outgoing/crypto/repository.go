	...
	for _, entry := range entries {
		...
		for _, ext := range extensions {
			if filepath.Ext(name) == ext {
				// Use fsutils.SafeJoin for path safety
				safePath, err := fsutils.SafeJoin(r.keyDir, name)
				if err != nil {
...
func (r FileSystemKeyRepository) ReadKeyFile(path string) ([]byte, error) {
	...
	// For private key files, also verify ownership (skip in CI environments)
	if level == security.Private {
		// Skip ownership check in CI environments
		if os.Getenv("CI") == "" {
