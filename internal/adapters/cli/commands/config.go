...
func printConfigText(effectiveConfig EffectiveConfig, output *os.File) (int, error) {
	...
	fmt.Fprintf(output, "  Body Min Lines: %d\n", cfg.Message.Body.MinLines)
	fmt.Fprintf(output, "  Allow Signoff Only: %t\n", cfg.Message.Body.AllowSignoffOnly)
	fmt.Fprintf(output, "  Require Signoff: %t\n", cfg.Message.Body.RequireSignoff)
	fmt.Fprintln(output)
	...
	fmt.Fprintln(output, "Signing Configuration:")
	fmt.Fprintf(output, "  Require Signature: %t\n", cfg.Signing.RequireSignature)
	fmt.Fprintf(output, "  Require Verification: %t\n", cfg.Signing.RequireVerification)
	fmt.Fprintf(output, "  Require Multi Signoff: %t\n", cfg.Signing.RequireMultiSignoff)

	if cfg.Signing.KeyDirectory != "" {
		fmt.Fprintf(output, "  Key Directory: %s\n", cfg.Signing.KeyDirectory)
	}

	if len(cfg.Signing.AllowedSigners) > 0 {
