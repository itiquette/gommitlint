...
func LoadConfigFromCommand(cmd *cli.Command) (ConfigResult, error) {
	...
	// Use repo-path for config discovery if specified
	cfg, err := config.LoadConfigWithRepoPath(repoPath)
	if err != nil {
		return ConfigResult{}, err
	}
	...
	if foundConfigFile := findExistingConfigFileInRepo(repoPath); foundConfigFile != "" {
		if repoPath != "" {
