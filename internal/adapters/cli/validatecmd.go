...
// runNewValidation handles the core validation logic using command pattern and returns an exit code.
func runNewValidation(ctx context.Context, cmd *cobra.Command, config config.Config) (int, error) {
	// Build command from CLI flags using pure function
	validationCmd, err := BuildValidationCommand(cmd, config)
	if err != nil {
		return 1, fmt.Errorf("failed to build validation command: %w", err)
	}

	// Execute the command using functional composition
	err = validationCmd.Execute(ctx)
	if err != nil {
