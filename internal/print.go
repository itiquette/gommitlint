...
func PrintReportTo(writer io.Writer, rules []model.CommitRule, commitInfo *CommitInfo) error {
	...
	for _, rule := range rules {
		if errs := rule.Errors(); len(errs) > 0 {
			for _, err := range errs {
				printRule(tw, rule.Name(), statusFailed, err.Error(), color.FgRed)
			}
			failed = true
		} else {
			printRule(tw, rule.Name(), statusPass, rule.Result(), color.FgGreen)
		}
	}
	...
	printRule(tw, "SUMMARY", "", summary+" "+result, summaryColor)
...
func printRule(w io.Writer, name string, status validationStatus, result string, statusColor color.Attribute) {
...
func countResults(rules []model.CommitRule) (passed, total int) {
	total = len(rules)
	for _, rule := range rules {
