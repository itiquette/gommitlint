...
func TestValidateJiraRule(t *testing.T) {
	...
	testCases := []struct {
		...
		// Conventional Commit Negative Cases
		{
			...
			isConventionalCommit: true,
			expectedErrors:       true,
			errorContains:        "no Jira issue key found in the commit message",
		},
		{
			...
			isConventionalCommit: true,
			expectedErrors:       true,
			errorContains:        "in conventional commit format, Jira issue key must be at the end of the first line",
		},
		{
			...
			isConventionalCommit: true,
			expectedErrors:       true,
			errorContains:        "Jira project UNKNOWN is not a valid project",
		},
		...
		{
			name:                 "Non-Standard Format But Valid",
			subject:              "This commit includes PROJ-123 in a non-standard way",
			isConventionalCommit: false,
			expectedErrors:       false,
		},
	...
	for _, tabletest := range testCases {
		t.Run(tabletest.name, func(t *testing.T) {
			...
			require.NotEmpty(t, result.Name(), "Name should not be empty")
