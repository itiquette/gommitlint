...
func TestSignedIdentity_VerboseResult(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			rule: SignedIdentity{
				errors: []*model.ValidationError{
					model.NewValidationError("SignedIdentity", "commit_nil", "commit cannot be nil"),
				},
			},
		...
		},
		{
			...
			rule: SignedIdentity{
				errors: []*model.ValidationError{
					model.NewValidationError("SignedIdentity", "no_key_dir", "no key directory provided"),
				},
			},
			...
			rule: SignedIdentity{
				errors: []*model.ValidationError{
					func() *model.ValidationError {
						err := model.NewValidationError("SignedIdentity", "invalid_key_dir", "invalid key directory: path error")
						_ = err.WithContext("error", "path error")
		...
		},
		{
			...
			rule: SignedIdentity{
				errors: []*model.ValidationError{
					model.NewValidationError("SignedIdentity", "no_signature", "no signature provided"),
				},
			},
		...
		},
		{
			...
			rule: SignedIdentity{
				...
				errors: []*model.ValidationError{
					func() *model.ValidationError {
						err := model.NewValidationError("SignedIdentity", "invalid_signature_format", "invalid format")
						_ = err.WithContext("error", "bad format")
		...
		},
		{
			...
			rule: SignedIdentity{
				...
				errors: []*model.ValidationError{
					func() *model.ValidationError {
						err := model.NewValidationError("SignedIdentity", "weak_key", "weak key")
						_ = err.WithContext("key_bits", "1024")
						_ = err.WithContext("required_bits", "2048")
