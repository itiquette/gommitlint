...
func (rule ImperativeVerb) VerboseResult() string {
	if len(rule.errors) > 0 {
		...
		case "non_verb":
			return "'" + rule.firstWord + "' is a non-verb word (article, pronoun, etc.). Use an action verb instead."

		case "past_tense":
			return "'" + rule.firstWord + "' is in past tense. Use present imperative form instead (e.g., 'Add' not 'Added')."

		case "gerund":
			return "'" + rule.firstWord + "' is a gerund (-ing form). Use present imperative form instead (e.g., 'Add' not 'Adding')."

		case "third_person":
			return "'" + rule.firstWord + "' is in third person form. Use present imperative form instead (e.g., 'Add' not 'Adds')."

		default:
			// If we have a first word but no specific code match, include it in the error
			if rule.firstWord != "" {
				return "Word '" + rule.firstWord + "' detected: " + rule.errors[0].Error()
			}
	...
	return "Commit begins with proper imperative verb '" + rule.firstWord + "'"
}
...
func (rule *ImperativeVerb) addError(code, message string, context map[string]string) {
	...
	if context != nil {
		if wordType, ok := context["type"]; ok {
			rule.verbType = wordType
		}

		if word, ok := context["word"]; ok {
			rule.firstWord = word
		} else if code == "invalid_conventional_format" {
			rule.firstWord = "(invalid format)"
		} else if code == "invalid_format" {
			rule.firstWord = "(invalid format)"
		} else if code == "missing_conventional_subject" {
			rule.firstWord = "(missing subject)"
		} else if code == "empty_message" {
			rule.firstWord = "(empty message)"
		} else if code == "no_first_word" {
			rule.firstWord = "(no valid first word)"
		}
	}
