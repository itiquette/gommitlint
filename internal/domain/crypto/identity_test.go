...
func TestIdentity(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			identity := crypto.NewIdentity(testCase.inputName, testCase.inputEmail)
...
func TestIdentityFromString(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			identity := crypto.NewIdentityFromString(testCase.input)
...
func TestIdentityMatches(t *testing.T) {
	identity1 := crypto.NewIdentity("John Doe", "john@example.com")
	identity2 := crypto.NewIdentity("John Smith", "john@example.com")
	identity3 := crypto.NewIdentity("Jane Doe", "jane@example.com")
	emptyIdentity := crypto.NewIdentity("", "")
...
func TestIdentityMatchesAny(t *testing.T) {
	...
	identities := []crypto.Identity{
		crypto.NewIdentity("Alice", "alice@example.com"),
		crypto.NewIdentity("Bob", "bob@example.com"),
		crypto.NewIdentity("John Smith", "john@example.com"),
	}

	require.True(t, identity1.MatchesAny(identities), "Should match one of the identities")

	noMatchIdentities := []crypto.Identity{
		crypto.NewIdentity("Alice", "alice@example.com"),
		crypto.NewIdentity("Bob", "bob@example.com"),
	}
