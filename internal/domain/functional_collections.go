...
// MapSliceCompat applies a function to each element of a slice and returns a new slice with the results.
// This is an alias for MapSlice to maintain compatibility with existing code that used util.Map.
func MapSliceCompat[T, U any](items []T, fn func(T) U) []U {
	return MapSlice(items, fn)
}

// FilterSliceCompat returns a new slice containing only elements that satisfy the predicate.
// This is an alias for FilterSlice to maintain compatibility with existing code that used util.Filter.
func FilterSliceCompat[T any](items []T, predicate func(T) bool) []T {
	return FilterSlice(items, predicate)
}
