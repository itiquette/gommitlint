		...
		},
		{
			...
			rule: SignedIdentity{
				errors: []*domain.ValidationError{
					domain.NewValidationError("SignedIdentity", "verification_failed", "verification failed"),
				},
			},
...
func TestSignedIdentity_VerboseResult(t *testing.T) {
	tests := []struct {
		name     string
		rule     SignedIdentity
		expected string
	}{
		{
			...
			expected: `Valid GPG signature from "Test User <test@example.com>"`,
		},
		{
			...
			expected: `Valid SSH signature from "ssh-key-user"`,
		},
		{
			name: "commit nil error",
			rule: SignedIdentity{
				errors: []*domain.ValidationError{
					domain.NewValidationError("SignedIdentity", "commit_nil", "commit cannot be nil"),
				},
			},
			expected: "Cannot verify signature: commit object is nil",
		},
		{
			name: "no key dir error",
			rule: SignedIdentity{
				errors: []*domain.ValidationError{
					domain.NewValidationError("SignedIdentity", "no_key_dir", "no key directory provided"),
				},
			},
			expected: "Cannot verify signature: no trusted key directory provided",
		},
		{
			name: "invalid key dir error",
			rule: SignedIdentity{
				errors: []*domain.ValidationError{
					func() *domain.ValidationError {
						err := domain.NewValidationError("SignedIdentity", "invalid_key_dir", "invalid key directory: path error")
						err = err.WithContext("error", "path error")

						return err
					}(),
				},
			},
			expected: "Cannot verify signature: invalid key directory - path error",
		},
		{
			name: "no signature error",
			rule: SignedIdentity{
				errors: []*domain.ValidationError{
					domain.NewValidationError("SignedIdentity", "no_signature", "no signature provided"),
				},
			},
			expected: "No cryptographic signature found on commit",
		},
		{
			name: "invalid signature format error",
			rule: SignedIdentity{
				SignatureType: GPG,
				errors: []*domain.ValidationError{
					func() *domain.ValidationError {
						err := domain.NewValidationError("SignedIdentity", "invalid_signature_format", "invalid format")
						err = err.WithContext("error", "bad format")

						return err
					}(),
				},
			},
			expected: "Invalid GPG signature format: bad format",
		},
		{
			name: "weak key error",
			rule: SignedIdentity{
				...
				errors: []*domain.ValidationError{
					func() *domain.ValidationError {
						err := domain.NewValidationError("SignedIdentity", "weak_key", "weak key")
						err = err.WithContext("key_bits", "1024")
						err = err.WithContext("required_bits", "2048")

						return err
					}(),
				},
			},
			expected: "Weak GPG key detected: 1024 bits (minimum required: 2048 bits)",
		},
	}

	for _, atest := range tests {
		t.Run(atest.name, func(t *testing.T) {
			assert.Equal(t, atest.expected, atest.rule.VerboseResult())
		})
	}
}
...
func TestVerifySignatureIdentity(t *testing.T) {
	...
	for _, tabletest := range tests {
		t.Run(tabletest.name, func(t *testing.T) {
			...
			if tabletest.expectError {
				...
				// Test both Result and VerboseResult methods
				assert.Equal(t, "Valid signature", result.Result(), "Result method returned unexpected value")
				assert.Contains(t, result.VerboseResult(), result.Identity, "VerboseResult should contain the identity")
				assert.Contains(t, result.VerboseResult(), result.SignatureType, "VerboseResult should contain the signature type")
			}
		})
	}
}
...
func TestSignedIdentity_Help(t *testing.T) {
	...
	for _, testCase := range errorCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := domain.NewValidationError("SignedIdentity", testCase.code, testCase.message)
