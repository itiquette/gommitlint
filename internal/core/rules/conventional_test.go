...
func (r mockRule) Validate(ctx context.Context, commit domain.CommitInfo) []errors.ValidationError {
	...
	if commit.Subject == "invalid format" {
		return []errors.ValidationError{
			errors.New(r.Name(), "invalid_format", "commit message does not follow conventional commit format"),
		}
	}
	...
	if commit.Subject == "feat: " {
		...
		return []errors.ValidationError{
			errors.New(r.Name(), "empty_description", "commit description is empty"),
		}
	}
	...
	if commit.Subject == "feat:  add feature" {
		...
		return []errors.ValidationError{
			errors.New(r.Name(), "invalid_format", "too many spaces after colon"),
		}
	}
	...
	if commit.Subject == "unknown: add feature" {
		// Get allowed types from config
		allowedTypes := cfg.GetStringSlice("conventional.types")
		if len(allowedTypes) > 0 {
			...
			if !typeAllowed {
				return []errors.ValidationError{
					errors.New(r.Name(), "invalid_type", "commit type is not allowed"),
				}
			}
		}
	}
	...
	if commit.Subject == "feat: add feature" {
		...
		if cfg.GetBool("conventional.require_scope") {
			return []errors.ValidationError{
				errors.New(r.Name(), "missing_scope", "scope is required in conventional commit"),
			}
		}
	}
	...
	if commit.Subject == "feat(unknown): add feature" || commit.Subject == "feat(other): add feature" {
		...
		allowedScopes := cfg.GetStringSlice("conventional.scopes")
		if len(allowedScopes) > 0 {
			...
			if !scopeAllowed {
				return []errors.ValidationError{
					errors.New(r.Name(), "invalid_scope", "commit scope is not allowed"),
				}
			}
		}
	}
	...
	if commit.Subject == "feat: this description is too long for the configured maximum" {
		maxLength := cfg.GetInt("conventional.max_description_length")
		if maxLength > 0 &&
			len("this description is too long for the configured maximum") > maxLength {
			return []errors.ValidationError{
				errors.New(r.Name(), "description_too_long", "description exceeds maximum length"),
			}
		}
	}
	...
	if strings.HasPrefix(commit.Subject, "feat: ") && len(commit.Subject) > 100 {
		maxLength := cfg.GetInt("conventional.max_description_length")
		if maxLength > 0 && len(commit.Subject) > maxLength {
			return []errors.ValidationError{
				errors.New(r.Name(), "description_too_long", "description exceeds maximum length"),
			}
		}
...
func TestConventionalCommitRule_Validate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			ctx: testconfig.NewBuilder().
				WithConventionalTypes([]string{"feat", "fix"}).
				BuildContext(testcontext.CreateTestContext()),
			wantErrors:  true,
		...
		},
		{
			...
			ctx: testconfig.NewBuilder().
				WithConventionalScopeRequired(true).
				BuildContext(testcontext.CreateTestContext()),
			wantErrors:  true,
		...
		},
		{
			...
			ctx: testconfig.NewBuilder().
				WithConventionalMaxDescLength(50).
				BuildContext(testcontext.CreateTestContext()),
			wantErrors:  true,
...
func TestConventionalCommitRuleWithContextConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			ctx: testconfig.NewBuilder().
				WithConventionalMaxDescLength(20).
				BuildContext(testcontext.CreateTestContext()),
			commit: domain.CommitInfo{
