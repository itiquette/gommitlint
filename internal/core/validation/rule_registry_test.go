...
func TestNewRuleRegistry(t *testing.T) {
	...
	// Create a new registry with nil analyzer
	registry := NewRuleRegistry(config, nil)
...
func TestRuleRegistry_GetRules(t *testing.T) {
	registry := NewRuleRegistry(DefaultConfiguration(), nil)
...
// TestRuleRegistry_WithAnalyzer tests that the analyzer is properly passed to rules.
func TestRuleRegistry_WithAnalyzer(t *testing.T) {
	// Create a mock analyzer
	mockAnalyzer := &MockCommitAnalyzer{}

	// Create registry with analyzer
	config := DefaultConfiguration()
	registry := NewRuleRegistry(config, mockAnalyzer)

	// Get the CommitsAhead rule
	rule := registry.GetRuleByName("CommitsAhead")
	require.NotNil(t, rule, "CommitsAhead rule should exist")

	// Validate the rule has the analyzer
	// Since we can't directly check internal fields, we'll verify indirectly
	// Either by checking the rule type or its behavior in a different test
	require.IsType(t, rules.CommitsAheadRule{}, rule, "Rule should be of CommitsAheadRule type")
}
...
// MockCommitAnalyzer is a mock implementation of domain.CommitAnalyzer.
type MockCommitAnalyzer struct{}

// GetCommitsAhead implements domain.CommitAnalyzer.
func (m *MockCommitAnalyzer) GetCommitsAhead(_ context.Context, _ string) (int, error) {
	panic("unimplemented")
}

func (m *MockCommitAnalyzer) GetCommitsAheadCount(_ string) (int, error) {
	// Return a mock value
	return 0, nil
}
